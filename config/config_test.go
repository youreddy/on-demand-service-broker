...
var _ = Describe("Config", func() {
	...
	Context("when the config file is valid YAML", func() {
		...
		Context("Credhub configuration", func() {
			Context("valid config", func() {
				BeforeEach(func() {
					configFileName = "credhub_config.yml"
				})
				...
				It("returns a config object", func() {
					Expect(*conf.Credhub).To(Equal(config.Credhub{
						APIURL: "example.com",
						ID:     "myclient",
						Secret: "mysecret",
					}))
			...
			Context("when not set", func() {
				BeforeEach(func() {
					configFileName = "credhub_off_config.yml"
				})
				...
				It("returns a nil object", func() {
					Expect(conf.Credhub).To(BeNil())
				})
			})

			Context("with no api url", func() {
				BeforeEach(func() {
					configFileName = "credhub_no_url_config.yml"
				})

				It("returns an error", func() {
					Expect(parseErr).To(MatchError(ContainSubstring("Must specify a Credhub api_url")))
				})
			})

			Context("with no client id", func() {
				BeforeEach(func() {
					configFileName = "credhub_no_client_id_config.yml"
				})

				It("returns an error", func() {
					Expect(parseErr).To(MatchError(ContainSubstring("Must specify a Credhub client_id")))
				})
			})

			Context("with no client secret", func() {
				BeforeEach(func() {
					configFileName = "credhub_no_client_secret_config.yml"
				})

				It("returns an error", func() {
					Expect(parseErr).To(MatchError(ContainSubstring("Must specify a Credhub client_secret")))
				})
			})
		})
